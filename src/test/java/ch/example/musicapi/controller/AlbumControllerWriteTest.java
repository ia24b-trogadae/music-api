...
class AlbumControllerWriteTest {
  ...
  void createAlbum_withValidBody_returnsCreated() throws Exception {
    ...
    mockMvc
        .perform(post("/albums").contentType(MediaType.APPLICATION_JSON).content(requestBody))
  ...
  void updateAlbum_whenAlbumExists_returnsUpdatedAlbum() throws Exception {
    ...
    mockMvc
        .perform(put("/albums/1").contentType(MediaType.APPLICATION_JSON).content(requestBody))
  ...
  void updateAlbum_whenAlbumDoesNotExist_returnsNotFound() throws Exception {
    ...
    mockMvc
        .perform(put("/albums/999").contentType(MediaType.APPLICATION_JSON).content(requestBody))
  ...
  void deleteAlbumById_whenAlbumExists_returnsNoContent() throws Exception {
    mockMvc.perform(delete("/albums/1")).andExpect(status().isNoContent());

    mockMvc.perform(get("/albums/1")).andExpect(status().isNotFound());
  }
  ...
  void deleteAlbumById_whenAlbumDoesNotExist_returnsNotFound() throws Exception {
    mockMvc.perform(delete("/albums/999")).andExpect(status().isNotFound());
  }

  @Test
  @DisplayName("DELETE /albums deletes all albums")
  void deleteAllAlbums_returnsNoContentAndEmptiesTable() throws Exception {
    mockMvc.perform(delete("/albums")).andExpect(status().isNoContent());
